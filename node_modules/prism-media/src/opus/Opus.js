...
class OpusStream extends Transform {
  ...
  constructor(options = {}) {
    if (!loadOpus().Encoder) {
      throw Error(
        "Could not find an Opus module! Please install @discordjs/opus, node-opus, or opusscript."
      );
    }
  ...
  _decode(buffer) {
    return this.encoder.decode(
      buffer,
      Opus.name === "opusscript" ? null : this._options.frameSize
    );
  }
...
class Encoder extends OpusStream {
  ...
  async _transform(chunk, encoding, done) {
    ...
    while (this._buffer.length >= this._required * (n + 1)) {
      const buf = await this._encode(
        this._buffer.slice(n * this._required, (n + 1) * this._required)
      );
      this.push(buf);
      n++;
    }
