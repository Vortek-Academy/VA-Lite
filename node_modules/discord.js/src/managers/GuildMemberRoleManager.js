...
class GuildMemberRoleManager {
  ...
  get hoist() {
    ...
    return hoistedRoles.reduce((prev, role) =>
      !prev || role.comparePositionTo(prev) > 0 ? role : prev
    );
  }
  ...
  async add(roleOrRoles, reason) {
    if (roleOrRoles instanceof Collection || Array.isArray(roleOrRoles)) {
      ...
      const newRoles = [
        ...new Set(roleOrRoles.concat(...this._roles.values())),
      ];
  ...
  async remove(roleOrRoles, reason) {
    if (roleOrRoles instanceof Collection || Array.isArray(roleOrRoles)) {
      ...
      const newRoles = this._roles.filter(
        (role) => !roleOrRoles.includes(role)
      );
      return this.set(newRoles, reason);
    } else {
      roleOrRoles = this.guild.roles.resolve(roleOrRoles);
