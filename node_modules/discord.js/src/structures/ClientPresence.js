...
class ClientPresence extends Presence {
  ...
  constructor(client, data = {}) {
    super(
      client,
      Object.assign(data, { status: "online", user: { id: null } })
    );
  }
    ...
    if (typeof presence.shardID === "undefined") {
      ...
      for (const shardID of presence.shardID) {
        this.client.ws.shards
          .get(shardID)
          .send({ op: OPCodes.STATUS_UPDATE, d: packet });
      }
    } else {
      this.client.ws.shards
        .get(presence.shardID)
        .send({ op: OPCodes.STATUS_UPDATE, d: packet });
    }
    return this;
  }
  ...
  async _parse({ status, since, afk, activity }) {
    const applicationID =
      ...
      (activity.application
        ? activity.application.id || activity.application
        : null);
    ...
    if (activity) {
      ...
      if (activity.assets && applicationID) {
        try {
          const a = await this.client.api.oauth2
        ...
        ? {
            ...
            assets: activity.assets
              ? {
                  ...
                  large_image:
                    assets.get(activity.assets.largeImage) ||
                    activity.assets.largeImage,
                  small_image:
                    assets.get(activity.assets.smallImage) ||
    ...
    if (packet.game) {
      packet.game.type =
        typeof packet.game.type === "number"
          ? packet.game.type
          : ActivityTypes.indexOf(packet.game.type);
    }
