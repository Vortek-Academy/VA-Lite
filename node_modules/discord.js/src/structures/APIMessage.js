...
class APIMessage {
  ...
  makeContent() {
    ...
    const isSplit =
      typeof this.options.split !== "undefined" && this.options.split !== false;
    const isCode =
      typeof this.options.code !== "undefined" && this.options.code !== false;
    ...
    if (content || mentionPart) {
      if (isCode) {
        const codeName =
          typeof this.options.code === "string" ? this.options.code : "";
        content = `${mentionPart}\`\`\`${codeName}\n${Util.cleanCodeBlockContent(
          content || ""
        )}\n\`\`\``;
        if (isSplit) {
          splitOptions.prepend = `${
            splitOptions.prepend || ""
          }\`\`\`${codeName}\n`;
          splitOptions.append = `\n\`\`\`${splitOptions.append || ""}`;
        }
  ...
  async resolveFiles() {
    ...
    this.files = await Promise.all(
      fileLikes.map((f) => this.constructor.resolveFile(f))
    );
    return this;
  }
  ...
  static transformOptions(content, options, extra = {}, isWebhook = false) {
    ...
    if (!options) {
      ...
      return isWebhook
        ? { content, embeds: [options], ...extra }
        : { content, embed: options, ...extra };
  ...
  static create(target, content, options, extra = {}) {
    ...
    const isWebhook =
      target instanceof Webhook || target instanceof WebhookClient;
    const transformed = this.transformOptions(
      content,
      options,
      extra,
      isWebhook
