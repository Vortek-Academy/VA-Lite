...
class Guild extends Base {
  ...
  addMember(user, options) {
    ...
    if (this.members.cache.has(user))
      return Promise.resolve(this.members.cache.get(user));
    options.access_token = options.accessToken;
    if (options.roles) {
      const roles = [];
      for (let role of options.roles instanceof Collection
  ...
  edit(data, reason) {
    ...
    if (typeof data.defaultMessageNotifications !== "undefined") {
      _data.default_message_notifications =
        typeof data.defaultMessageNotifications === "string"
          ? DefaultMessageNotifications.indexOf(
    ...
    return this.client.api
      ...
      .then(
        (newData) => this.client.actions.GuildUpdate.handle(newData).updated
      );
  }
  ...
  equals(guild) {
    ...
    if (equal) {
      if (this.embedChannel) {
        if (
          !guild.embedChannel ||
          this.embedChannel.id !== guild.embedChannel.id
        )
          equal = false;
